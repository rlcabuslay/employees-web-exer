...
public class EmployeeDAO {
    private static final String SELECT_ALL_EMPLOYEES = "SELECT emp.employee_id, emp.first_name, emp.last_name, emp.email, emp.salary, "
    ...
    private Employee toEmployee(ResultSet row) throws SQLException {
        ...
        employee.setDept_id(row.getInt(DEPT_ID_COLUMN));
        employee.setDept_name(row.getString(DEPT_NAME_COLUMN));
        employee.setJob_id(row.getString(JOB_ID_COLUMN));
        employee.setJob_title(row.getString(JOB_TITLE_COLUMN));
        return employee;
    }
}
